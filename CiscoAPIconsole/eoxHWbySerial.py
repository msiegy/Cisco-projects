...
information to a CSV.
    ...
    PsuedoCode: For Each SW Release In 'csvInputPath' Run get_eoxbySerial and append EoS, LDoS, etc tuples in 'csvWritePath'
"""
...
def writecsv(PID, EoS, EoM, LDoS, URL, MigrationPID, MigrationURL, MigrationStrat):
    ...
    with open(csvWritePath, 'ab') as csvfile:
        fieldnames = ['PID', 'EoS', 'EoM', 'LDoS', 'EoX URL', 'MigrationPID', 'MigrationURL', 'MigrationStrat']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        if not file_exists:
            writer.writeheader()

        writer.writerow({'PID': PID, 'EoS': EoS, 'EoM': EoM, 'LDoS': LDoS, 'EoX URL': URL, 'MigrationPID': MigrationPID, 'MigrationURL': MigrationURL, 'MigrationStrat': MigrationStrat})
